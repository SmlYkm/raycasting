...
namespace Game
{
    Player::Player(Math::Vector2f pos, float ang, float vel, float angVel)
        : position(pos), angle(ang), velocity(vel), angularVelocity(angVel)
    {}
    ...
    void Player::handleInput()    // Keyboard input
    {
        ...
        if(sf::Keyboard::isKeyPressed(sf::Keyboard::W))
            Math::Physics::updatePosition(position, velocity, angle);
